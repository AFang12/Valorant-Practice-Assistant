...
class YOLOv8:
    ...
    def __init__(self, onnx_model, input, confidence_thres, iou_thres, yaml_path=None, save_image=False):
        """
        ...
        self.input = input
        if 'image' in filetype.guess(self.input).mime:
    ...
    def main(self):
        ...
        Returns:
            result: {fps: int 每秒帧数,res: list res[i]是第i帧的检测结果}
            fps=0时,代表输入为单个图像
        """
        ...
        if 'video' in filetype.guess(self.input).mime:
            ...
            outputs = session.run(None, {model_inputs[0].name: img_data})
            img, outputs = self.postprocess(self.img, outputs)
            # Perform post-processing on the outputs to obtain output image.
            return {'fps': 0, 'res': outputs}  
